...
def prepare_afhq(cfg, logger, train=True):
	if train:
	...
	source_path = './datasets/afhq/train'
	# for filename in tqdm.tqdm(os.listdir(source_path)):
	for filename in tqdm.tqdm(list(glob(f'{source_path}/*/*.*'))):
		if is_image_file(filename):
			images.append((0, filename))
	...
	afhq_folds = [[] for _ in range(folds)]

	count_per_fold = count // folds
	for i in range(folds):
		afhq_folds[i] += images[i * count_per_fold: (i + 1) * count_per_fold]
	...
	for i in range(folds):
		...
		for label, filename in tqdm.tqdm(afhq_folds[i]):
			...
			for lod in range(cfg.DATASET.MAX_RESOLUTION_LEVEL, 1, -1):
				...
				image_down = F.avg_pool2d(image, 2, 2).clamp_(0, 255).to('cpu', torch.uint8).view(3,
																								  image.shape[2] // 2,
																								  image.shape[
																									  3] // 2).numpy()
