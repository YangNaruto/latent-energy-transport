...
class Solver(object):
	def __init__(self, args):
		...
		elif args.dataset.lower() in ['celeba_hq', 'afhq', 'celeba']:
			self.nc = 3
			self.decoder_dist = 'gaussian'
		else:
		...
		self.ckpt_dir = os.path.join(args.ckpt_dir, os.path.join(args.viz_name, args.dataset))
		...
		self.output_dir = os.path.join(args.output_dir,os.path.join(args.viz_name, args.dataset))
		...
		while not out:
			for x in self.data_loader:
				...
				if self.global_iter % self.display_step == 0:
					if recon_loss.item() < self.best_mse:
						self.best_mse = recon_loss.item()
						self.save_checkpoint('best')

					pbar.write('[{}] recon_loss:{:.3f} total_kld:{:.3f} mean_kld:{:.3f}'.format(
						self.global_iter, recon_loss.item(), total_kld.item(), mean_kld.item()))

					# var = logvar.exp().mean(0).data[0]
					# var_str = ''
